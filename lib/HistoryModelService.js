...
var self = {
  createModels: function createModels(sails){
    try {
      processCreateModels(sails);
    } catch(e) {
      sails.log.error('Error creating history models');
    }

    function processCreateModels(sails) {
      async.auto({
        loadCollection: createAllModelHistory,
        tearDown: ['loadCollection', tearDownDatabase],
    ...
    function createAllModelHistory(asyncCb) {
      if (!sails.models) {
        return asyncCb({
          message: 'No models!'
        });
    ...
    function tearDownDatabase(asyncCb){
